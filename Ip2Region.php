...
class Ip2Region
{
     ...
     * ```
     */
    private function getSearcher(string $ip): Searcher
    {
     ...
     * ```
     */
    private function createSearcher(string $version): Searcher
    {
        try {
            // 获取数据库文件
            ...
            switch ($this->cachePolicy) {
                case 'vectorIndex':
                    ...
                    return Searcher::newWithVectorIndex($ipVersion, $file, $vectorIndex);
                case 'content':
                    // 内容缓存模式需要先读取整个文件
                    $content = file_get_contents($file);
                    return Searcher::newWithBuffer($ipVersion, $content);
                case 'file':
                default:
